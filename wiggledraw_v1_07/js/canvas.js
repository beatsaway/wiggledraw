...
class CanvasManager {
    ...
    drawIsometricGrid(ctx, x0, y0, w, h, spacing) {
        ...
        for (let x = -h * tan60; x < w + h * tan60; x += spacing) {
            ctx.beginPath();
        ...
        for (let x = w + h * tan60; x > -h * tan60; x -= spacing) {
            ctx.beginPath();
        ...
        for (let x = firstX; x < w; x += spacing / 2) {
            ctx.beginPath();
    ...
    drawObliqueGrid(ctx, x0, y0, w, h, spacing) {
        ...
        for (let x = w + h * tan45; x > -h * tan45; x -= spacing / 2) {
            ctx.beginPath();
