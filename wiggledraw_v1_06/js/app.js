    ...
    setupUIControls() {
        ...
        document.getElementById('loadExampleBtn').addEventListener('click', async () => {
            ...
            if (value === 'blank') {
                ...
                window.strokeHistory = [];
                window.canvasManager.ctx.clearRect(0, 0, window.canvasManager.canvas.width, window.canvasManager.canvas.height);
                window.drawingManager.saveCanvasState();
            } else {
                // Load example JSON from examples folder
                try {
                    const response = await fetch(`examples/${value}`);
                    ...
                    if (json.width && json.height && (json.width !== window.canvasManager.canvas.width || json.height !== window.canvasManager.canvas.height)) {
                        const scaleX = window.canvasManager.canvas.width / json.width;
                        const scaleY = window.canvasManager.canvas.height / json.height;
